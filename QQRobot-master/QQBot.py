...
logging.basicConfig(filename=None, level=logging.DEBUG, format='%(asctime)s  %(filename)s[line:%(lineno)d] %(levelname)s %(message)s', datefmt='%a, %d %b %Y %H:%M:%S')
...
def uin_to_account(tuin):
    ...
    if tuin not in FriendList:
        try:
            ...
            logging.info(u"Get uin to account info:" + str(info))
            if info['retcode'] != 0:
    ...
    logging.info(u"Now FriendList:" + str(FriendList))
            ...
            if targetThread:
                ...
                try:
                    ...
                    if msgType == 'sess_message':
                        ...
                        logging.info(u"Get group sig:" + str(info))
                        if info['retcode'] != 0:
                            raise ValueError, info
                        info = info['result']
        ...
        if msgType == 'group_message':
            ...
            if str(gid) in GroupWatchList:
                ...
                if g_exist:
                    ...
                    logging.info(u"群线程已生成")
            else:
                logging.info(str(gid) + u"群有动态，但是没有被监控")
    ...
    def __init__(self, vpath, qq=0):
        ...
        while LoginError > 0:
            try:
                ...
                logging.info(u"getvfwebqq html:  " + str(html2))
...
class pmchat_thread(threading.Thread):
    ...
    def __init__(self, tuin, isSess, group_sig, service_type):
        ...
        self.lastseq=0
        self.replystreak = 0
        logging.info(u"私聊线程生成，私聊对象："+str(self.tqq))
    def check(self):
        self.lastcheck = time.time()
    def run(self):
        while 1:
    ...
    def reply(self, content):
        ...
        logging.info(u"Reply to " + str(self.tqq) + ":" + str(content))
    ...
    def push(self, ipContent, seq):
        ...
        try:
            ...
            logging.info(u"PM get info from AI: "+ipContent)
            ...
            logging.info(u"AI REPLY:"+str(info))
...
class group_thread(threading.Thread):
    ...
    def reply(self, content):
        if time.time() - self.lastreplytime < 3.0:
            logging.info(u"REPLY TOO FAST, ABANDON："+content)
            return False
        ...
        logging.info(u"Reply package: " + str(data))
        rsp = HttpClient_Ist.Post(reqURL, data, Referer)
        try:
            rspp = json.loads(rsp)
            if rspp['errCode'] == 0:         
                logging.info(u"[Reply to group " + str(self.gid) + "]:" + str(content))
                return True
        except:
            pass
    ...
    def handle(self, send_uin, content, seq):
        ...
        print 'hadle message:', send_uin, content, seq
    ...
    def repeat(self, content):
        if self.last1 == str(content) and content != '' and content != ' ':
            if self.repeatPicture or "[图片]" not in content:
                self.reply("复读机:"+content)
                logging.info(u"已复读：{" + str(content) + "}")
                return True
        self.last1 = content
    ...
    def callout(self, send_uin, content):
        ...
        try:
            if match:
                logging.info(u"get info from AI: "+str(match.group(2)).decode('UTF-8'))
    ...
    def aboutme(self, content):
        ...
        try:
            if match:
                logging.info(u"output about info")
    ...
    def deleteall(self, content):
        ...
        try:
            if match:
                logging.info(u"Delete all learned data for group:"+str(self.gid))
                info="已删除所有学习内容"
